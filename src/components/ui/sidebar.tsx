  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          "fixed inset-y-0 z-50 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-300 ease-out md:flex",
          side === "left"
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="neutral"
      size="icon"
      className={cn("size-7 cursor-pointer relative z-10", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-out group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",

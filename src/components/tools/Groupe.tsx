...
function DraggableStudent({ student, groupIndex }: { student: User; groupIndex: number }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`font-medium justify-content items-center flex gap-2 bg-gray-200 p-1 rounded-md cursor-grab active:cursor-grabbing ${
...
function DroppableGroupCard({ groupIndex, children }: { 
  group: User[]; 
  groupIndex: number; 
  ...
  return (
    <Card 
      ...
      className={`border-2 bg-white p-1 overflow-hidden transition-colors ${
        isOver ? 'border-chart-1 bg-blue-50' : ''
      }`}
    >
...
export default function Groupe() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (sourceGroupIndex === targetGroupIndex || targetGroupIndex === undefined) return;
    ...
    newGroups[targetGroupIndex] = [...newGroups[targetGroupIndex], activeStudent];
  ...
  const createGroupe = (selectedStudents: User[], tailleGroupe: number) => {
    ...
    for (let i = 0; i < nombreGroupes; i++) {
      ...
      const tailleDuGroupe = i < groupesAvecPersonneSupplementaire ? tailleBase + 1 : tailleBase;
        ...
        <DragOverlay>
          {activeStudent ? (
            <div className="font-medium justify-content items-center flex gap-2 bg-chart-1 p-1 rounded-md shadow-lg border-2 border-chart-1">
              ...
              <p className="truncate">{activeStudent.name}</p>
            </div>
          ) : null}
        </DragOverlay>
      </DndContext>
